...
\end{equation}
where $\vc{q}$, $\vc{\dot{q}}$ and $\vc{\ddot{q}}$ are the position, velocity and acceleration in generalized coordinates. $\vc{M(q)}$ is the mass matrix\index{mass matrix}. $\vc{C(q,\dot{q})}$ accounts for the Coriolis and centrifual force. $\boldsymbol{\tau}$ is the external generalized force\footnote{Due to convention, the same symbol $\boldsymbol{\tau}$ is used to represent torque in maximal coordinates and generalized force in generalized coordinate. However, in most of cases, they are two different quantities.}, including gravity and contact force. $\boldsymbol{\tau}^a$ is the generalized force exerted by the controller\index{controller} (Section 4). This equation can be derived from Lagrangian dynamics. Detailed derivation is omitted here but can be found in \citet{Liu:2012:STM}.
